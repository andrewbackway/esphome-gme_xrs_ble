    ...
    case ESP_GATTC_OPEN_EVT: {
      ...
      if (param->open.status == ESP_GATT_OK) {
        ESP_LOGI(TAG, "GATT connection opened (conn_id=%d)",
                 param->open.conn_id);
        // Pairing and encryption are handled by ble_client + YAML automations.
      } else {
    ...
    case ESP_GATTC_WRITE_CHAR_EVT: {
      ...
      if (param->write.status != ESP_GATT_OK) {
        ...
        if (param->write.status == ESP_GATT_INSUF_AUTHENTICATION ||
          ...
          ESP_LOGI(TAG,
                   "Write failed due to authentication; pairing must be "
                   "completed via ble_client automations");
        }
      }
...
void GmeXrsRadioComponent::handle_notify_(uint16_t handle, const uint8_t* data,
  ...
  ESP_LOGV(TAG, "Notification from handle 0x%04X (known=%s, len=%u)", handle,
...
void GmeXrsRadioComponent::ensure_paired_() {
  ESP_LOGD(TAG, "ensure_paired_: pairing handled by ble_client / YAML automations");
}
